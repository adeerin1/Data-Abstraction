...
ofstream out("horn_results_time.txt");
...
bool fillMap(HashMap<double, Data, std::function<unsigned(double)>> &hash, unordered_map<double, Data> &data)
{
	...
	if (hash.size() != data.size())
		return false;
	for (auto &p : data)
	{
		if (data[p.first] != p.second)
			return false; // change to hash
	}
	return true;
}
...
int doTests()
{
	HashMap<int, string, std::function<int(int)>> hash([](int i)
													   { return i; });
	...
	HashMap<int, string, std::function<int(int)>> hash2([](int i)
														{ return i; });
	hash2[16] = "six-teen";
	hash2[3] = "three";
	hash2[2] = "two";
	...
	out << "simple passed" << endl;
	cout << "Simpled tests passed." << endl;
	...
	for (auto &i : keys_to_erase)
	{
		if (um2.erase(i) != hash2.erase(i))
		{
			cout << "Disagreement on erase of " << i << endl;
			return 2;
		}
	}
